...
![Factory Method pattern](images/Algorithm/factory-method-en.png)

**使用场景**

jdbc 连接数据库，硬件访问，降低对象的产生和销毁。

1. 当你不能提前知道你的代码需要一起工作的对象的类型和依赖时，可以使用工厂方法。
   - 工厂方法分离产品创建方法代码和使用产品的代码，这样就可以很容易的扩展产品创建的代码而不影响其他使用产品的代码。例如给程序增加一个新的产品类型，你只需要增加一个新的创建者子类并实现工厂方法。
2. 当你需要为用户提供一个用户可扩展内部组件的库或者框架时，可以使用工厂方法。
3. 当你需要重用已经存在的对象而不是每次都重新创建新资源，从而节省系统资源时可以使用工厂方法。如数据连接sessionFactory.
...
| 好处                             | 坏处                                                         |
| ---------------------------------------- | ---------------------------------------------------- |
| 避免创建者和具体产品紧密耦合 | 代码或许变得更复杂，因为你引入了很多新的子类。<br/>最好的场景是在已有对象结构基础上引入工厂方法。 |
| 符合单一职责原则。           |                                                              |
| 符合开闭原则                 |                                                              |
...
- **抽象工厂**通常基于一组**工厂方法**
- **工厂方法**可以和**迭代器**一块使用，让集合的子类返回兼容集合接口的不同类型的迭代器
- **工厂方法**是**模板方法**的一个特列，工厂方法可能是一个大的模板方法中的一步。
...
![Abstract Factory pattern](images/Algorithm/abstract-factory-en.png)

**使用场景**

当有一系列产品族，并且你不想依赖具体的产品，这些产品可能不能提前知道或者说你想让这些产品将来是可以扩展的。
...
| 好处                                       | 坏处                             |
| ------------------------------------------ | -------------------------------- |
| 你可以确定从工厂获取的多个产品是相互兼容的 | 代码更复杂，引入更多的接口和子类 |
| 解除了和具体产品代码的紧密耦合             |                                  |
| 符合单一职责和开闭原则                     |                                  |

**和其他设计模式的关系**

- **建造者模式**关注一步步创建复杂对象，**抽象工厂**更擅长创建相关的一系列产品。**抽象工厂**立即返回产品，**建造者模式**需要客户端代码做一些额外的构建步骤才能返回产品。
